import { Matrix4, Matrix3, InstancedBufferAttribute, InstancedBufferGeometry } from 'three';

/**
 * Effect for rendering in HoloJS framework (https://github.com/Microsoft/HoloJS)
 * required for advanced mode only (not auto-stereo)
 * @author Stuart Anderson <stuart.anderson@data61.csiro.au>
 */
export class HolographicEffect {

    constructor (renderer) {
        this.renderer = renderer;

        this._modelViewMatrix = [new Matrix4(), new Matrix4()];
        this._normalMatrix = [new Matrix3(), new Matrix3()];
        ...
        for (var name in attributes) {
            ...
            if (attribute.isInstancedBufferAttribute) {
                ...
                if (!holographicAttribute) {
                    holographicAttribute = holographicAttributes[name] = new InstancedBufferAttribute(attribute.array, attribute.itemSize, attribute.meshPerAttribute * 2);
    ...
    _initHolographicGeometry (object) {
        ...
        if (holographicGeometry === undefined) {
            holographicGeometry = object.geometry.__holographicGeometry = new InstancedBufferGeometry();
            ...
            holographicGeometry.addAttribute('aRenderTargetArrayIndex', new InstancedBufferAttribute(this._renderTargetArrayIndexBuffer, 1, holographicGeometry.maxInstancedCount / 2));
        }
        else {
            if (geometry.isBufferGeometry) this._updateBufferGeometry(holographicGeometry, geometry);
